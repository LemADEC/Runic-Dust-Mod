...
public class ErfBendInscription extends InscriptionEvent {
	...
	public ErfBendInscription(int[][] design, String idName, String properName, int id) {
		...
		if (((EntityPlayer) wearer).getCurrentEquippedItem() == null && wearer.isSneaking() && wearer.onGround) {
			try {
				...
				if (!isAlreadyLaunching(item) && target != null && (buttons[0] || buttons[1])) {
					...
					if ((world.isAirBlock(x, y, z) && world.isAirBlock(x, y - 1, z)) || target.isInWater())
						return;
					if (buttons[0] && checkForEarth(target, r + 1, launch) < 0.80)
						return;
		...
		for (int i = -r; i <= r; i++) {
			for (int j = -r; j <= r; j++) {
				for (int k = 0; k > -launch; k--) {
					...
					if (block.getMaterial() == Material.air && k != 0)
	...
	public boolean isSolid(Block block, int meta) {
		...
		if (m == Material.vine || m == Material.ice || m == Material.wood || m == Material.web)
			return false;
		if (m == Material.snow) {
	...
	public Block getCompressedMaterial(Block block, int meta) {
		...
		if (block == Blocks.stone || block == Blocks.grass || block == Blocks.dirt || block == Blocks.gravel || block == Blocks.double_stone_slab || block == Blocks.stone_slab
				|| block == Blocks.mossy_cobblestone || block == Blocks.clay || block == Blocks.stonebrick)
	...
	public Block getFillerMaterial(Block block, int meta) {
		if (block == Blocks.sand || block == Blocks.sandstone)
			return Blocks.sand;
		if (block == Blocks.stone || block == Blocks.gravel || block == Blocks.double_stone_slab || block == Blocks.stone_slab || block == Blocks.mossy_cobblestone || block == Blocks.clay
	...
	public boolean isCompressedMaterial(Block block) {
		Block[] blocks = new Block[] { Blocks.sandstone, Blocks.cobblestone, Blocks.mossy_cobblestone, Blocks.netherrack };
	...
	public static synchronized Entity getLaunchEnt(EntityPlayer entityPlayer) {
		try {
			float var9 = 1.0F;
			double var11 = 48; //reach
			Vec3 var6 = Vec3.createVectorHelper(entityPlayer.posX, entityPlayer.posY, entityPlayer.posZ);//ep.getPosition(0);
			Vec3 var7 = entityPlayer.getLook(0);
			Vec3 var8 = var6.addVector(var7.xCoord * var11, var7.yCoord * var11, var7.zCoord * var11);
			List var10 = entityPlayer.worldObj.getEntitiesWithinAABBExcludingEntity(entityPlayer,
					entityPlayer.boundingBox.addCoord(var7.xCoord * var11, var7.yCoord * var11, var7.zCoord * var11).expand(var9, var9, var9));
			...
			if (pointedEntity != null && entityPlayer.canEntityBeSeen(pointedEntity))
			...
			for (int j = 1; j >= -1; j--) {
				for (int k = 1; k >= -1; k--) {
					...
					if (isCompressedMaterial(block)) {
						...
						EntityBlock eb = new EntityBlock(world, x + 0.5, y + 0.5, z + 0.5, block);
						eb.setThrowing(true);
						//						eb.noClip = true;
