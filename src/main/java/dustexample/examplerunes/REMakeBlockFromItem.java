 ...
 */
public class REMakeBlockFromItem extends RuneEvent
{

    public REMakeBlockFromItem()
    {
        super();
    }
    ...
    public void onInit(EntityRune e)
    {
        ...
        if (this.takeItems(e, new ItemStack(Items.iron_ingot, 1)))
        {
            e.data[0] = 1;
        } else if (this.takeItems(e, new ItemStack(Items.gold_ingot, 1)))
        {
            e.data[0] = 2;
        } else if (this.takeItems(e, new ItemStack(Items.diamond, 1)))
        {
            e.data[0] = 3;
        } else
        {
        ...
        if(e.data[1] < 8 && e.ticksExisted % 20 == 0){
            ...
            switch (e.data[0]){
                case 1:
                    ...
                    break;
                case 2:
                    blockID = Blocks.gold_block;
                    break;
                case 3:
                    blockID = Blocks.diamond_block;
                    break;
            }
            
            world.setBlockToAir(x,y,z); //clears the block first. Sometimes a good idea since it could be a TileEntity
