	...
	public static void checkInscriptionImage(InscriptionEvent event) {
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				...
				if (value != 0) {
					int[] loc = getPosition(x, y, dotWidth, spaceWidth, bgw, bgh, width, height);
					...
					if (x < width - 1 && values[x + 1 + y * width] == value) {
						int[] nextLoc = getPosition(x + 1, y, dotWidth, spaceWidth, bgw, bgh, width, height);
					...
					if (y < height - 1 && values[x + (y + 1) * width] == value) {
						int[] nextLoc = getPosition(x, y + 1, dotWidth, spaceWidth, bgw, bgh, width, height);
