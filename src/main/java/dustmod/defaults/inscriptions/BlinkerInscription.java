...
public class BlinkerInscription extends InscriptionEvent {
	...
	public BlinkerInscription(int[][] design, String idName, String properName, int id) {
	...
	public void onUpdate(EntityLivingBase wearer, ItemStack item, boolean[] buttons) {
		...
		if (((EntityPlayer) wearer).getCurrentEquippedItem() == null && wearer.isSneaking()) {
			...
			DustMod.spawnParticles(wearer.worldObj, "reddust", Math.floor(testLoc[0]) + 0.5, newY, Math.floor(testLoc[2]) + 0.5, canTele ? -1 : 0, canTele ? 0.8 : 0, canTele ? 0.8 : 0, 2, 0.5, 0.1,
					0.5);
