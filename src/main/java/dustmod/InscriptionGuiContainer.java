	...
	public InscriptionGuiContainer(InventoryPlayer inventoryPlayer, IInventory inv){
		...
		for(int i = 0; i < 16*16; i++){
		 ...
		 for(int i = 1; i < 10; i++){
			 ...
			 if(stack != null && stack.getItem() == DustMod.ink){
		        	int id = ItemInk.getDustID(stack);
		        	this.putStackInSlot(0, new ItemStack(DustMod.ink, id, i-1));
				 break;
			 }
		 }
	}
...
//            }
	...
	public boolean canInteractWith(EntityPlayer p) {
		...
		return p.getCurrentEquippedItem().getItem() == DustMod.inscription;
	}
