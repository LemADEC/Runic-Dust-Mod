...
public class EntityRune extends Entity {
	...
	/**
	 * The datawatcher is 100% dedicated to the visual aspects of the dust entity. This is because only the server needs to know anything involving the logic (the data array is not synced) and the
	 * client just needs to know what the player needs to see.
	 */
	@Override
	...
	public void setEvent(RuneEvent evt, String name) {
		event = evt;
		eventName = name;
		this.updateDataWatcher();
	}
	...
	protected void readEntityFromNBT(NBTTagCompound tag) {
		...
		if (tag.hasKey("entityDustID")) {
			...
			EntityRuneManager.registerEntityDust(this, entityDustID);
		} else {
			entityDustID = -1L;
		}
		if (tag.hasKey("rot")) {
			runeRotation = tag.getInteger("rot");
		}
		...
		event = RuneManager.get(eventName);
	...
	protected void writeEntityToNBT(NBTTagCompound tag) {
		...
		tag.setInteger("rot", runeRotation);
	...
	/**
	 * Used to slowly kill the rune. During fading, the animations still go but no ticks are called to the event. Typically used when a rune is finished and ends
	 */
	public void fade() {
		fade = true;
	}
	...
	/**
	 * Immediately kills the rune and spawns smoke particles. Typically used to indicate a failed sacrifice or something similar.
	 */
	public void fizzle() {
		doFizzle = true;
	}
